...
interface SmokePO { poNumber: string; customer: string; style: string; quantity: number; currency: string }
const expected: SmokePO = { poNumber: 'TEST-001', customer: 'Demo Customer', style: 'ABC123', quantity: 1200, currency: 'USD' };
const schema: SchemaValidator<SmokePO> = {
  name: 'qimo_preview_smoke_v1', strict: true,
  jsonSchema: {
...
export const POST = createSmokePostHandler({
  ...
  async execute() {
    ...
    const result = await new OpenAIAdapter().generateObject({
      scene: 'preview.internal.structured-smoke', capability: 'structured-extraction',
      ...
      schema, timeoutMs: 20_000, maxOutputTokens: 64, fallback: 'disabled',
    }, model);
    const fields = Object.keys(expected) as Array<keyof SmokePO>;
    return {
      provider: 'openai' as const, model: result.model, latencyMs: result.latencyMs, usage: result.usage,
      schemaValid: true, fieldMatchCount: fields.filter(field => result.data[field] === expected[field]).length,
    };
  },
});
