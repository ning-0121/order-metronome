 ...
 * agent-scan 的职责由现行 cron 覆盖(morning-briefing→daily-briefing;daily-summary→reminders 督办日报;
...
export async function GET() {
  return NextResponse.json(
    { disabled: true, reason: 'agent-scan disabled — zombie route retired (audit 2026-07-28)' },
    { status: 503 },
  );
}
