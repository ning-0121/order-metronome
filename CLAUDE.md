...
orders            — 订单主表
milestones        — 里程碑（模板版本化:V2标准15/V1托底11/打样8;is_critical 物化于行）
...
**模板已版本化(单一真相 `lib/milestoneTemplate.ts`,仅新单生效,在途订单不动):**
- **V2 标准生产模板 = 15 节点**(2026-07 起新单;业务执行节拍,含订单评审会/包装方式确认,移除生产启动)
- **V1 托底 = 11 节点**(在途/历史订单)
- **打样模板 = 8 节点**
- 节点是否"关键"以模板物化的 `is_critical` 为准(交付置信度引擎读它,非硬编码;2026-07-21 收口双轨)

概念阶段(供理解,实际节点以模板为准):
- **阶段A 订单启动**：PO审查确认 → PI客户确认 → 生产单/原辅料单 → 订单评审会 → 采购核料提交
- **阶段B 产前样**：产前样寄出 → 产前样确认
- **阶段C 生产出货**：中期验货 → 包装方式确认 → 尾期验货
- **阶段D 出运**：船样 → CI/报关 → 订舱 → 发货出运 → 收款
...
- 里程碑模板完整性（V2标准=15节点，V1托底≥11，打样=8节点）
