...
export const CHECKLIST_MAP: Record<string, ChecklistConfig> = {
  ...
  materials_received_inspected: {
    ...
    items: [
      ...
      { key: 'shrinkage', label: '缩水率', type: 'select', required: false, role: 'merchandiser', group: '品质检验',
        options: ['合格(≤3%)', '偏差较大(需确认)', '不合格', '暂无法确认(大货未到)'],
        helpText: '寄办公室的样布测不出缩水率 → 可不确认;客户要求时再测大货' },
      { key: 'defect_check', label: '疵点检查', type: 'select', required: false, role: 'merchandiser', group: '品质检验',
        options: ['无明显疵点', '有少量(可用)', '大面积疵点(需退换)', '暂无法确认(大货未到)'],
        helpText: '大货未到无法查瑕疵点 → 可不确认;客户要求时再验大货' },
  ...
  packing_method_confirmed: {
    ...
    items: [
      ...
      { key: 'pcs_per_carton', label: '每箱件数', type: 'number', required: true, role: 'merchandiser', group: '装箱数据',
        helpText: '一律按【件】填(系统全链路按件驱动)。套装单请折件:如 3件套 每箱30套 → 填 90;备注可写"30套×3件"' },
      { key: 'total_cartons', label: '总箱数', type: 'number', required: true, role: 'merchandiser', group: '装箱数据' },
      { key: 'weight', label: '净重/毛重(KG)', type: 'text', required: false, role: 'merchandiser', group: '装箱数据',
        helpText: '大货未完成时可先确认包装方式、净毛重后补;大货完成后回填' },
