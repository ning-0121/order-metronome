...
export async function getProductionCenter(): Promise<{
  ...
  if (!roles.some((r) => ['production', 'production_manager', 'merchandiser', 'order_manager', 'qc', 'quality', 'admin'].includes(r))) {
    return { error: '无权查看生产中心' };
  }
  // 生产经理/理单/管理层看全部;生产(非经理)/跟单只看分配到自己的单
  // 2026-07-20 审计:QC/质检天然要看所有待验货的单,原 owner 过滤会让 QC 看到空页,无法工作。
  // 2026-07-30:原来这里用 `|| roles.some(r => ['qc','quality'])` 就地打补丁,现收敛进
  //   CAN_VIEW_ALL_ORDERS(该组 = 管理层 + QC,专管订单可见性,不含审批/邮件/督办)。
  const canSeeAll = roles.includes('admin') || hasRoleInGroup(roles, 'CAN_VIEW_ALL_ORDERS');
