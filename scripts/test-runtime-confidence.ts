...
section('Case 9 — 4 个关键节点同时超期（数据维护差） → 递减叠加，避免 0%');
{
  ...
  // 旧逻辑：4 × -30 = -120 → 0%
  // 新逻辑：worst -25 + 叠加项被距离软化 → ≈ 60% orange
  assert(out.confidence > 30, '不应被惩罚到 < 30%（旧逻辑会到 0%）', `got ${out.confidence}`);
  assert(out.confidence <= 70, '应仍在 orange 范围（确实危险但有恢复时间）', `got ${out.confidence}`);
  // 至少显示 2 条递减叠加（第 4 条可能因 -40 cap 被略过，符合预期）
  ...
  assert(totalCriticalDeduction >= -40, `critical_step_overdue 总扣分 ≥ -40：实际 ${totalCriticalDeduction}`);
  // 第一条是 worst（用 100% factor，base 25，不软化）
  assert(criticalReasons[0].delta === -25, `worst 扣 -25：实际 ${criticalReasons[0].delta}`);
  // 第二条 50% × 0.6 (距出厂 > 30 天软化) = ~-8
  assert(criticalReasons[1].delta >= -10 && criticalReasons[1].delta <= -6,
    `2nd 在 -10 到 -6 之间（叠加项被距离软化）：实际 ${criticalReasons[1].delta}`);
}
