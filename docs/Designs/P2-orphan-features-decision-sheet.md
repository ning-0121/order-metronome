...
| A2 | **采购对账 / 退货 / 付款申请**(ProcurementReconciliationPanel,25 handler + 9 action) | PO 详情页注释"保留未删,要恢复加回一行即可"。三张表全 0 行 = 从未用过。**昨天上的「付款待回执」审批类别会恒 0,因为建付款申请的唯一入口就是这个孤儿面板** | 加回一行 + 验流程,半天~1 天 | **恢复**(否则对账/付款闭环只存在于设想中;若决定付款全走外部财务系统,则删并同时撤掉 payment_request 类别) | ☐ |
