...
const success: SmokeExecutionResult = {
  ...
  schemaValid: true, fieldMatchCount: 5,
};
...
describe('Preview-only AI smoke endpoint', () => {
  ...
  it('pins the endpoint to direct adapter execution with retry and fallback disabled', () => {
    const source = readFileSync(join(process.cwd(), 'app/api/internal/ai-runtime-smoke/route.ts'), 'utf8');
    assert.equal(source.includes('new OpenAIAdapter()'), true);
    assert.equal(source.includes("fallback: 'disabled'"), true);
    assert.equal(source.includes('qimoAI.'), false);
  });
});
