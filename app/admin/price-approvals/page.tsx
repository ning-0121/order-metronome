...
export default function PriceApprovalsPage() {
  ...
  const visible = filter === 'pending' ? items.filter(i => isApprovalPending(i.status)) : items;
  const stats = {
    pending: items.filter(i => isApprovalPending(i.status)).length,
                ...
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {expired && isApprovalPending(it.status) && (
