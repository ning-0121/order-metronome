...
完整登记表见 [shared-core-registry.md](./shared-core-registry.md)。下表为概要：
...
| **Progressive Validation 模式** | `app/actions/orders.ts` + `app/actions/milestones.ts` + `lib/services/daily-tasks.service.ts` | 渐进式字段校验骨架，**不含 Qimo 专属数据**，可直接同步 |
