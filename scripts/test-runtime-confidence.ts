...
section('Case 9 — 4 个关键节点同时超期（数据维护差） → 递减叠加，避免 0%');
{
  ...
  // 第一条是 worst（用 100% factor，base 25）
  assert(criticalReasons[0].delta === -25, `worst 扣 -25：实际 ${criticalReasons[0].delta}`);
  // 第二条 50%（base 25 × 0.5 = -13 四舍五入）
  assert(criticalReasons[1].delta === -13 || criticalReasons[1].delta === -12,
    `2nd 扣 -13 左右（50%）：实际 ${criticalReasons[1].delta}`);
}
