...
// svc 读写(2026-08-19 P0):accessory_import_candidates / materials_bom 的 RLS 对
// 跟单/采购/业务 session 全滤(user_can_access_order 不含他们,采购侧旁路在生产库未生效)——
// 导入整批被 WITH CHECK 拒、候选列表恒空、批量通过恒 0 条、批准建 BOM 行被拒。
// 各函数入口已有 CAN_EDIT_BOM 门禁,表访问统一走 service-role。
...
export async function importAccessoryCandidates(orderId: string, attachmentId: string, bytes: ArrayBuffer) {
  ...
  const { error } = await (svcDb().from('accessory_import_candidates') as any).insert(inserts);
...
export async function listAccessoryCandidates(orderId: string, status?: string) {
  ...
  { const err = await requireRoleGroup(sb, user.id, 'CAN_EDIT_BOM', '仅业务/理单/采购/管理可查看辅料候选'); if (err) return { error: err }; }
  let q = (svcDb().from('accessory_import_candidates') as any).select('*,order_attachments(file_name,file_url)').eq('order_id', orderId).order('source_row_number');
...
export async function reviewAccessoryCandidate(id: string, orderId: string, action: 'approve'|'exclude', approvedValue?: Record<string, unknown>, reason?: string) {
  ...
  const { data: c } = await (svcDb().from('accessory_import_candidates') as any).select('*').eq('id', id).eq('order_id', orderId).single();
  if (!c) return { error: '候选行不存在或无权访问' };
  if (action === 'exclude') {
    const value = { ...(c.extracted_value || {}), exclusion_reason: String(reason || '人工排除').slice(0, 200) };
    const { error } = await (svcDb().from('accessory_import_candidates') as any).update({ import_status: 'EXCLUDED', approved_value: value, reviewed_by: user.id, updated_by: user.id, reviewed_at: new Date().toISOString() }).eq('id', id);
  ...
  if (!c.matched_bom_id) {
    let duplicateQuery = (svcDb().from('materials_bom') as any).select('id').eq('order_id', orderId)
    ...
    const { data: made, error: makeErr } = await (svcDb().from('materials_bom') as any).insert({ order_id: orderId, material_name: value.accessory_name,
  ...
  const { error } = await (svcDb().from('accessory_import_candidates') as any).update({ import_status: 'APPROVED', approved_value: value,
  ...
  if (error && createdBomId) await (svcDb().from('materials_bom') as any).delete().eq('id', createdBomId);
...
export async function bulkApproveExactCandidates(orderId: string) {
  ...
  const { data } = await (svcDb().from('accessory_import_candidates') as any).select('id,extracted_value,missing_fields')
