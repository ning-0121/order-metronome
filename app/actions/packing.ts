...
// ── 装箱单页(PackingTab)已删(2026-08-19 CEO:装箱职能已被「出货单据」流覆盖)──
// PackingTab 专属导出 getPackingLists/addPackingList/addPackingLine/deletePackingLine/confirmPackingList 随之移除。
// 本文件保留出货单据流(ShippingDocsSection 在用):getShippingDraft / saveShippingDocMeta / saveShippingLines。
// packing_list_lines 仍是出货事实的存储层(出货录入写它),只是不再有独立「装箱单」录入/确认 UI。
