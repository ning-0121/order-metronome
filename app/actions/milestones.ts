...
export async function markMilestoneDone(
  ...
  if (milestone.step_key === 'pi_confirmed' && !isAdmin) {
    ...
    if (!hasPrice) {
      return {
        error: '完成 PI 客户确认前必须先录客户成交价(否则财务只能建 ¥0 空壳预算、采购会被卡)。录入口:打开本订单「🏭 生产任务单」tab → 逐款明细里「成交价」列填客户单价 → 保存,再回来点完成。(仅财务/业务/管理员可见该列)',
      };
    }
  }
    ...
    if (kind === 'production' && !hasRoleNodes) {
      ...
      return {
        ...
        message: `已指派${roleLabel}。本单用的是旧版节点模板、没有生产部门线节点,`
          // 别写"本页":这条消息现在也会在排产工作台弹出(2026-08-21 派单修复后透传),
          // 而「重建为最新节点模板」按钮只在订单详情页的进度校准里,说"本页"会让人在排产页空找。
          + `因此负责人记在订单上;要让生产跟单有节点可推进,需到该订单详情页用「重建为最新节点模板」升级节点。`,
      } as any;
    }
