...
export function ProcurementItemsTab({ orderId, focusItemId, internalOrderNo }: { orderId: string; focusItemId?: string | null; internalOrderNo?: string | null }) {
  ...
  const [sizeOrderPref, setSizeOrderPref] = useState<string[] | null>(null);   // 业务手排的尺码顺序(富录入表)
  useEffect(() => { getOrderSizeOrder(orderId).then(r => setSizeOrderPref((r as any).sizeOrder ?? null)); }, [orderId]);
  ...
  const consMissing = consLines.filter(l => l.required && !(Number(l.production_consumption) > 0) && !notSelf(l.id));
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="text-left text-gray-400">
                {['款号', '颜色', '物料', '类型', '供料方式', '数量', '开发单耗', '大货单耗', '预算单价(业务填)', '抛量%(采购填)', '单位'].map(h => (
                  <th key={h} className="py-1.5 px-2 font-medium whitespace-nowrap">{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {consLines.map(l => (
                  <tr key={l.id} className={`border-t border-gray-100 ${l.required && !(Number(l.production_consumption) > 0) && !notSelf(l.id) ? 'bg-amber-50' : ''} ${notSelf(l.id) ? 'bg-sky-50/60' : ''}`}>
                    ...
                    {/* 供料方式:自购=绮陌买;客供=客户供;加工厂承担=工厂承担费用。后两者绮陌都不采购/不计成本,仅生产任务单辅料明细标注 */}
                    <td className="py-1.5 px-2 whitespace-nowrap">
                      <select value={supplyEdit[l.id] || 'self'} disabled={trackingPhase}
                        onChange={e => setSupplyEdit(prev => ({ ...prev, [l.id]: e.target.value as any }))}
                        title="自购=绮陌采购;客供=客户供料;加工厂承担=费用由加工厂承担。后两者绮陌不采购、不计成本,只在生产任务单辅料明细标注给生产/财务监督"
                        className={`rounded border px-1.5 py-1 text-xs ${notSelf(l.id) ? 'border-sky-300 bg-sky-50 text-sky-700 font-medium' : 'border-gray-300 text-gray-600'}`}>
                        <option value="self">绮陌自购</option>
                        <option value="customer">客供</option>
                        <option value="factory">加工厂承担</option>
                      </select>
                    </td>
                  <td className="py-1.5 px-2 font-medium text-gray-800" title="该款×色数量基准">{l.quantity_display ?? l.pieces ?? '—'}</td>
                    ...
                    <td className="py-1.5 px-2">
                      {notSelf(l.id) ? <span className="text-[11px] text-sky-600">{supplyEdit[l.id] === 'factory' ? '加工厂承担' : '客供'}·绮陌不采购</span>
                        : l.required ? (<>
          ...
          {styleBudgets.length > 0 && (
            <div className="rounded-lg border border-indigo-200 bg-indigo-50/50 p-3 mt-2 space-y-2">
              ...
              <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-indigo-100">
                ...
                <span className="text-xs text-gray-500">元/套 × {orderQuantityDisplay || (commercialOrderQty != null ? `${commercialOrderQty.toLocaleString()} 套` : (budgetOrderQty > 0 ? `${budgetOrderQty.toLocaleString()} 件` : '?'))}</span>
                {Number(accessoryPerPiece) > 0 && (commercialOrderQty ?? budgetOrderQty) > 0 && (
                  <span className="text-xs font-mono font-semibold text-indigo-700">= ¥{(Math.round(Number(accessoryPerPiece) * (commercialOrderQty ?? budgetOrderQty) * 100) / 100).toLocaleString()}</span>
                )}
      ...
      {sel && (
        <div className="rounded-xl border border-indigo-200 bg-indigo-50/30 p-4 space-y-4">
          ...
          {splittable && sel && (() => {
            ...
            const sizes = orderSizeKeys(sizeSet, sizeOrderPref);   // 业务手排优先,回落标准码序(别按录入先后)
