/**
 * 小绮对话 — 上下文/提示构建(共享层)。
 * askAgent(server action,非流式兜底)与 /api/agent-chat(流式主路径)共用,
 * 保证两条路径喂给 Claude 的内容完全一致。
 */
...
export async function buildAgentChatPayload(
  supabase: any,
  user: any,
  question: string,
  ...
  if (question.includes('今天') || question.includes('今日') || question.includes('待办') || question.includes('我的')) {
    ...
    if ((roleFilter as any).owner_role) query = query.eq('owner_role', (roleFilter as any).owner_role);
