...
import {
  getQuickEntries, STAGE_DEFINITIONS,
  type DashboardLink, type DashboardRole, type DetailedProductionTask,
...
// QC 只关心"待验/待放行/风险",采购·物料·排产口径与 QC 无关(2026-07-20 审计:去掉无关 KPI 减负)
const QC_KPI = [
  ['in_production', '生产中·待验', '◇', 'text-indigo-700'],
  ['ready_to_ship', '待发货·待放行', '△', 'text-teal-700'],
  ['risk', '风险单', '!', 'text-rose-700'],
] as const;
...
export function ProductionCenterClient({
  summary, dashboard, canManage, role = 'follow_up', initialDetail = '', initialStage = '',
}: { summary: ProductionCenterSummary; dashboard: DashboardData; canManage: boolean; role?: DashboardRole; initialDetail?: string; initialStage?: string }) {
  ...
  const quickEntries = getQuickEntries(role).map((entry, index) => ({ ...entry, icon: quickEntryIcons[index] }));
  const kpiCards = role === 'qc' ? QC_KPI : KPI;
  const detailLabel = role === 'qc' ? '我的质检任务' : role === 'follow_up' ? '我的跟单任务' : '生产主管详细任务';
  ...
  return (
    <div className="space-y-4">
      ...
      <QimoKpiGrid>
        {kpiCards.map(([key, label, icon, tone]) => (
          <QimoKpiCard key={key} href={`/production?stage=${key}#details`} label={label} value={summary[key as keyof ProductionCenterSummary]} icon={<span className="text-sm font-semibold">{icon}</span>} tone={tone} />
        ))}
      </QimoKpiGrid>
      ...
      <section id="details" className="rounded-xl border border-gray-200 bg-white">
        <button type="button" onClick={() => { const next = !open; setOpen(next); if (next && tasks.length === 0) load(true); }} className="flex w-full items-center gap-2 px-4 py-3 text-left hover:bg-gray-50" aria-expanded={open}>
          <span className={`text-xs text-gray-400 transition ${open ? 'rotate-90' : ''}`}>▶</span><span className="text-sm font-semibold text-gray-900">{detailLabel}（{total}）</span><span className="ml-auto text-xs text-indigo-600">{open ? '收起' : '展开'}</span>
        </button>
