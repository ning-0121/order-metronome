...
**在干净网络（美国/无 GFW）操作迁移。** 中国 + VPN(fake-IP)环境下实测:Docker 容器连不上 Supabase、
直连库 IPv6-only 不可达、本机无 pg_dump 且装不了——处处受阻。回美国后 pg_dump/pg_restore/Docker 全畅通,
标准流程一次跑通。

⚠️ **操作者位置 ≠ 数据区域**:哪怕从美国操作,新建 Supabase 项目的 **Region 仍选 `East Asia (Hong Kong)`**
(这才是给中国团队提速的目的)。美国操作 + 香港落地。
