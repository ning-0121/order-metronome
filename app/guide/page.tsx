...
const ROLE_DAILY_SOPS = [
  ...
  },
  {
    ...
    label: '财务',
    color: 'green',
    responsibilities: 'PO 审核、加工费确认、出货审批、收款',
    milestones: ['财务审核', '加工费确认', '核准出运', '收款完成'],
    dailySteps: [
      '打开"我的工作台"，查看待处理的财务节点',
      ...
      '加工费到位后核对并上传确认函',
      '在「出货管理」中审批出货申请：',
      '  - 查看业务提交的出货详情（数量/客户/送货方式/CI等）',
      '  - 确认付款状态（已收款/部分收款/信用放行）',
      '  - 批准放行 或 驳回（需填写原因）',
      '收款完成后标记最后节点',
    ],
    tips: [
      '财务审核直接影响后续所有节点启动，请优先处理',
      '出货审批流程：验货放行 → 业务申请 → 财务审批 → 物流执行',
      '驳回出货时必须填写原因，业务可修改后重新申请',
  ...
  },
  {
    ...
    label: '物流/仓库',
    color: 'sky',
    responsibilities: '出货执行、装柜运输、提单获取',
    milestones: ['出运'],
    dailySteps: [
      '在「出货管理」中查看财务已审批的出货单',
      '协调货车装柜，确认装柜时间和地点',
      '填写执行信息：实际出货日期、提单号、船名/航次、柜号',
      '拍摄装柜照片（空柜→装货→满柜→封柜）',
      '确认出货完成 → 自动锁定',
    ],
    tips: [
      '出货流程：验货放行 → 业务申请 → 财务审批 → 物流执行',
      '装柜照片需拍摄：空柜、装货过程、满柜、封柜',
      '国内送仓订单：获取签收单/回单后确认完成',
      '如遇运输异常，及时通知业务和管理员',
    ],
  },
  {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">{BRAND.productName} — 员工操作手册</p>
      </div>
      ...
      <section id="updates" className="mb-10">
        ...
        <div className="space-y-4">
          ...
          {/* 建单资料 + 预算口径(2026-07-08 改版) */}
          <div className="rounded-xl border border-green-200 bg-green-50/30 p-4">
            <h3 className="text-sm font-bold text-green-800 mb-2">🔍 建单资料 + 成本预算</h3>
            <ul className="text-sm text-gray-700 space-y-1.5">
              <li>• 创建订单只需上传 <strong>客户PO</strong>(可选,也可手工录逐款明细)+ <strong>尺码表</strong>;<strong>不再上传报价单</strong>。</li>
              <li>• 成本/预算改由业务在订单的「<strong>采购核料</strong>」逐料手填:面料 大货单耗×预算单价、逐款 加工费 + 辅料单件总价。</li>
              <li>• 预算 vs 实际下单的对比在「采购成本核算 → 预算vs实际」,数据即来自采购核料,超预算标红。</li>
            </ul>
          </div>
      ...
      <section id="faq" className="mb-10">
        ...
        <div className="space-y-2">
          {[
            ...
            { q: '成本预算怎么录？', a: '2026-07 起建单不再上传报价单。成本/预算改由业务在订单「采购核料」逐料手填：面料 大货单耗×预算单价、逐款 加工费 + 辅料单件总价。预算 vs 实际下单的对比在「采购成本核算 → 预算vs实际」，超预算标红。' },
      ...
      <div className="text-center py-6 border-t border-gray-200">
        <p className="text-xs text-gray-400">{BRAND.productName} | 访问 {BRAND.siteDomain} | 如有问题请联系管理员</p>
      </div>
    </div>
  );
}
